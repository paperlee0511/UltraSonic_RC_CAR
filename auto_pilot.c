...
void Auto_Pilot_Update(void)
{
	...
	// ✅ 0. 세 방향 모두 여유 → 전진
	if(front_distance >= 22 && left_distance >= 13 && right_distance >= 13)
	...
	// ✅ 1. 세 방향 모두 막힘 → 후진 후 좌우 비교
	else if(front_distance <= 22 && left_distance <= 21 && right_distance <= 21)
	...
	// ✅ 2. 전방 + 우측 막힘 → 좌회전
	else if(front_distance <= 24 && right_distance <=32)
	{
		if(now - last_action_time >= ACTION_INTERVAL)
		{
			Motor_back_control(MOTOR_SPEED_90, MOTOR_SPEED_90);
	...
	// ✅ 3. 전방 + 좌측 막힘 → 우회전
	else if(front_distance <= 24 && left_distance <= 32)
	{
		if(now - last_action_time >= ACTION_INTERVAL)
		{
			Motor_back_control(MOTOR_SPEED_90, MOTOR_SPEED_90);
	...
	// ✅ 4. 대각선 벽 접근 (측면 가까움) → side_diff 기반 회피
	else if(front_distance >= 3 && (left_distance <= 35 || right_distance <= 35))
	{
		if(now - last_action_time >= ACTION_INTERVAL)
		{
			Motor_back_control(MOTOR_SPEED_90, MOTOR_SPEED_90);
